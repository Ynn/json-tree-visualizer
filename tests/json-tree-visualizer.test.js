...
describe('JSONTreeVisualizer (global bundle)', () => {
  it('exposes the expected properties', () => {
    ...
    console.log('Exposed keys on JSONTreeVisualizer:', Object.keys(globalThis.JSONTreeVisualizer));
    expect(globalThis.JSONTreeVisualizer).toBeTypeOf('object');
  });

  it('generates valid DOT (convertToDot)', () => {
    const dot = globalThis.JSONTreeVisualizer.convertToDot(myData, { colorScheme: 'vibrant', maxArrayItems: 20 });
  ...
  it('generates SVG without error', async () => {
    const svg = await globalThis.JSONTreeVisualizer.renderSVG(myData, { colorScheme: 'vibrant', maxArrayItems: 20 });
